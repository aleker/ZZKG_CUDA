...
__global__ void count_bits(unsigned int *inputMatrix, float *resultMatrix)
{
    ...
    unsigned int v = inputMatrix[tid]; //wartość do sprawdzenia
    v = v - ((v >> 1) & 0x55555555);
    v = (v & 0x33333333) + ((v >> 2) & 0x33333333);
    int c = ((v + (v >> 4) & 0xF0F0F0F) * 0x1010101) >> 24; //wynik
    resultMatrix[tid] = c;
}
...
int main() {
    ...
    // kernel
    count_bits << <dimGrid, dimBlock >> > (d_a.data().get(), d_b.data().get());
